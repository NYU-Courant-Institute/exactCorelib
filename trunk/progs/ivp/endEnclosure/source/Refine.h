  ...
  //  cout << "q" << q << endl;
   ...
   // cout << "spe:" << S.E[idx] << endl;
    ...
    for (int j = 1; j < N + 1; ++j) {
        ...
        if (debug == 1) {
            for (int i = 0; i < dim; ++i) {
                if (q[i].leftBound() < p3(S.G[idx].bfE[j])[i].leftBound() || q[i].rightBound() > p3(S.G[idx].bfE[j])[i].rightBound()) {
                    ...
                    cout << "H" << H << endl;
                    cout << "q" << q << endl;
                    cout << p3(S.G[idx].bfE[j-1]) << endl;
                    cout << p3(S.G[idx].bfE[j]) << endl;
        ...
        S.G[idx].bfF[j] = IntersectB(S.G[idx].bfF[j],Itmp );
        ...
        if (debug == 1) {
            for (int i = 0; i < dim; ++i) {
                double lower = std::max(S.G[idx].bfE[j][i].leftBound(), inp3(q + B1)[i].leftBound());
                double upper = std::min(S.G[idx].bfE[j][i].rightBound(), inp3(q + B1)[i].rightBound());
                // If the intersection is empty, return an empty interval
                if (lower > upper) {
        ...
        Itmp = IntersectB(S.G[idx].bfE[j], inp3(q + B1));
        if (debug == 1) {
            for (int i = 0; i < dim; ++i) {
        ...
        S.G[idx].bfE[j] = IntersectB(Itmp, inp3(q) + B);
...
void Refinenewsimple(Stage& S, std::vector<std::string> SVar, int dim, IMap f, int stepBtype, int degree, double veps, int debug) {
    double r0 = wmax(S.E.back());
    while (veps < r0) {
        IVector E0(dim);
        for (int i = 0; i < dim; ++i) {
...
void RefinenewsimpleT(Stage& S, std::vector<std::string> SVar, int dim, IMap f, int stepBtype, int degree, double veps,int debug) {
    double r0 = wmax(S.E.back());
    while (veps < r0) {
        IVector E0(dim);
        for (int i = 0; i < dim; ++i) {
...
void Refinenewnotransform(Stage& S, std::vector<std::string> SVar, int dim, IMap f, int stepBtype, int degree, double veps,int debug) {
    double r0 = wmax(S.E.back());
    while (veps < r0) {
        IVector E0(dim);
        for (int i = 0; i < dim; ++i) {
...
void Refinenewnoeuler(Stage& S, std::vector<std::string> SVar, int dim, IMap f, int stepBtype, int degree, double veps, int debug) {
    double r0 = wmax(S.E.back());
    while (veps < r0) {
        IVector E0(dim);
        for (int i = 0; i < dim; ++i) {
