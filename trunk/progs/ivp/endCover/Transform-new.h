...
// 替换字符串中的所有变量
string replaceAll(std::string f, const std::string target, const std::string replacement) {
    if (target.empty()) return f; // 避免死循环
    size_t pos = 0;
    while ((pos = f.find(target, pos)) != std::string::npos) {
        f.replace(pos, target.length(), replacement);
        pos += replacement.length(); // 移动到替换后的结尾继续搜索
    }
...
// 主函数：输入 SFun 和区间矩阵 Q，返回结果向量
vector<string> transformSFun(const vector<string>& SFun, const vector<string>& SVar, IMatrix& Q,int n) {
   // IMatrix TQ = transpose(Q);
    // Step1: 计算 Q*SVar
    vector<string> QSVar(n);
    for (int i = 1; i <= n; i++) {
        std::stringstream expr;
        expr << "(";
        for (int j = 1; j <= n; j++) {
            double coeff = Q(j, i).leftBound(); // 获取左边界值
            if (j >= 1) {
                if (coeff >= 0) expr << "+";
                else expr << "-";
            }
            expr << std::abs(coeff) << "*" <<SVar[j - 1];
        }
        expr << ")";
        QSVar[i - 1] = expr.str();
    }
    //cout << QSVar << endl;
    // Step2: 把 Q*SVar 代入 SFun
    vector<string> SFun_sub(SFun.size());
    for (int i = 0; i < SFun.size(); i++) {
        string f = SFun[i];
        for (char& c : f) {
            c = std::toupper(static_cast<unsigned char>(c));
        }
        for (int j = 0; j < n; j++) {
            f = replaceAll(f, toUpper(SVar[j]), QSVar[j]);
        }
        SFun_sub[i] = expand_expression(f);
    }
  // cout <<" SFun_sub=" << SFun_sub << endl;
    // Step3: 再乘一次 Q * SFun_sub
    vector<string> result(n);
    for (int i = 1; i <= n; i++) {
        stringstream expr;
        for (int j = 1; j <= n; j++) {
            double coeff = Q(i, j).leftBound(); // 获取左边界值
            if (j >= 1) {
                if (coeff >= 0) expr << "+";
                else expr << "-";
            }
            expr << abs(coeff) << "*(" << SFun_sub[j - 1] << ")";
        }
        result[i - 1] = expand_expression(expr.str());
    }
