...
\begin{enumerate}
      ...
      \item \textbf{Cylinder} : $C=\{(x,y,z)|A=-x\sin(\theta)+y\cos(\theta)\cos(\phi)+z\cos(\theta)\sin(\phi), B=-y\sin(\phi)+ z\cos(\phi),A^2+B^2\le R^2\}$.
...
To a polyhedron $P \subset \mathbb{R}^3$, we use set V, E and F to represent its vertices, edges and faces respectively. The \textbf{features} $\Phi(P) = (V, E, F)$.
\\
\\
...
\indent  Given a box $B_0$ and polyhedron $P$, we subdivide $B_0$ recursively and to each subbox we compute $\Phi^*\ (B, P)$. During the subdivision process, we get the \textbf{subdivision tree(S-Tree)} for $P$ rooted at $B_0$. A subdivision tree is an octree, in which each node keeps the info of $\Phi^*\ (B, P)$. 
\\
\\
...
\\
\\
\indent We define $T = SD (B_0, P)$. \begin{comment}in which SD is a function on a  box $B$ and a polyhedron $P$ \end{comment}
\\
\\
\indent Each node n of AS-Tree has the following info:
\begin{itemize}
	\item pointer to the parent of n(to $B_0$)
	\item pointer to 

\end{itemize}
      	
\subsection{Boolean Operations on AS-Tree}
 ...
 %$T = SD (B_0, P)$ in which SD is a function on a  box $B$ and a polyhedron $P$, the root of the tree  points to box $B_0$.  All leaves of $T$ point to an elementary box of the polyhedron $P$.
		...
		Boolean operations on two adequate subdivision trees $T_1 = SD (B_1, P_1)$ and $T_2 = SD (B_2, P_2)$: 
		$T = M (BO, T_1, T_2)$ in which BO is a Boolean operation  and  $T_1$ and $T_2$ are adequate subdivision trees. The result of the function $M$ is an adequate subdivision tree $T$.\\
		if $B_1 \not= B_2$, $T=Null$.\\
