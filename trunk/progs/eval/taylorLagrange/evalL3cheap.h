...
#ifndef __CORE_EVALL3CHEAP_H__
#define __CORE_EVALL3CHEAP_H__

#include "evalLagrange.h"
#include "exactRange.h"

class evalL3cheap : public evalLagrange {
public:
  ...
  // CONSTRUCTORS
  //
  evalL3cheap(vector<real> v, interval I): evalLagrange(v,I) { name = "evalL'3"; }
  evalL3cheap(): evalLagrange() { name = "evalL'3"; }
  ...
  //
    ...
    //
    //    []R_{h_0}(I) = [-1,1] T'_{3,n},
    //
    // where T'_{3,n} = \sum_{j=1}^{d/3} |[]h_j(I)| \Omega_3^j
    // and the range of h_j is estimated with the centered form evaluation
    //
    // compute T'_{3,n} using Horner summation
    //
  ...
  //       size T'_{3,n} are already available
  //
