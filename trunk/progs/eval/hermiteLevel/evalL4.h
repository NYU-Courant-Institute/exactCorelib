...
#ifndef __CORE_EVALL4_H__
#define __CORE_EVALL4_H__

#include "evalLagrange.h"
#include "exactRange.h"

class evalL4 : public evalLagrange {
public:
  ...
  // CONSTRUCTORS
  //
  evalL4(vector<real> v, interval I): evalLagrange(v,I) { name = "evalL4"; }
  evalL4(): evalLagrange() { name = "evalL4"; }
  ...
  //
    ...
    //
    //    []R_{h^_0}(I) = [-1,1] T_{4,n},
    //
    // where T_{4,n} = |h^_1(I)| \Omega_3 + \sum_{j=2}^{d/3} |h_j(I)| \Omega_3^j
    //
    ...
    //
    // get the exact range of h^_1
    //
    h1I = quadraticRange( 0,d11,d12, r);

    //
    // add |h^_1(I)| to current T and multiply with \Omega_3
    //
    real T4 = ( T + max(fabs(h1I.a),fabs(h1I.b)) ) * Omega3;
  ...
  //       remainder size T_{3,n} (all but the first term of this sum)
  ...
  //
    ...
    //
    // recall that  h_1(x) = h^_1(x) + f'''(m),
    // hence h_1(I) = h^_1(I) + f'''(m)
    //
    h1I.a += f3m;
    h1I.b += f3m;

    //
    // add |h_1(I)| to cashed T and multiply with \Omega_3
    //
    T = ( T + max(fabs(h1I.a),fabs(h1I.b)) ) * Omega3;
