...
function add_optimized_instruction!(slp::SLP, op::Symbol, left_idx::Union{Int, String}, right_idx::Union{Int, String})::Union{Int, String}
    ...
    # Check for existing identical instructions
    for (i, instruction) in enumerate(slp.codelist)
        out_idx, existing_op, existing_left, existing_right, _ = instruction
        if existing_op == op && existing_left == left_idx && existing_right == right_idx
            return out_idx
        end
        if (op == :+ || op == :*) && existing_op == op && existing_left == right_idx && existing_right == left_idx
            return out_idx
        end
    end
