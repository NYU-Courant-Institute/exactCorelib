...
void isolateRoots(Polynomial<T>& P, BFVecInterval& v)
{
  ...
  //R = Polynomial<T>(n, temp1);
  //cout << "The negative polynomial "; R.dump();cout << endl;
...
int main( int argc, char* argv[] ) {
  ...
  //BigFloat coeff[]={1,5,-6,3,0.7,8,9.5,1,0,10,1};
  //P =Poly(10, coeff);
  P.dump(); cout <<endl;
  /*for(int i=0; i <= P.getTrueDegree(); i++){
    if(P.coeff[i].err() != 0)
      cout <<"ERROR!!! Input Coefficients have error" << endl;
      }*/

  BFVecInterval v;
  isolateRoots(P, v);
  cout <<"Upper Bound "<< P.CauchyBound() << endl;
  //v.clear();
  //BFInterval I(0, 3.5);
  //isolateRoots(P, I, v);
