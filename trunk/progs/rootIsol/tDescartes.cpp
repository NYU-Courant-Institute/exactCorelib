...
int main( int argc, char* argv[] ) {
  ...
  //P = Poly("x^3 - 3*x^2 + 2*x"); Polynomial with zero as a root
  BFVecInterval v;
  BigInt B=CauchyBound(P);
  BFInterval I;
  if(argc > 3 )
    I=BFInterval(BigFloat(argv[2]), BigFloat(argv[3]));
  else
    I=BFInterval(-B, B);
  ...
  if(Iter > 0)
    testDescartes2(P, I , Iter);
  else
    isolateRoots(P, I, v);
  //isolateRoots(P, v);
  //cout <<"Upper Bound "<< CauchyBound(P) << endl;
  //v.clear();
  //BFInterval I(0, 3.5);
  //isolateRoots(P, I, v);
  ...
  /*  BigFloat a=0;
  I = firstRootAbove(P, a);
  cout <<"First root above " << a << "is in the interval ["<< I.first 
       << ":" << I.second << "]"<< endl;

  I = firstRootBelow(P, a);
  cout <<"First root below " << a << "is in the interval ["<< I.first 
       << ":" << I.second << "]"<< endl;
  */
  /*  Sturm<NT> S(P);
  S.dump();cout<<endl;
  cout<<"Length of the sequence -1 is "<< S.len << endl;
  cout << "sign variations at pos infty "<< S.signVariationsAtPosInfty() <<endl;
  cout << "sign variations at neg infty "<< S.signVariationsAtNegInfty() <<endl;
  cout <<"Number of roots of P"<< S.numberOfRoots() <<endl;
  */  
