/***************************************************
  file: prover2.cc
   ...
   $Id: prover.cpp,v 1.1 2006-03-07 04:51:25 exact Exp $
 ***************************************************/
...
int Prover::solveQuadr(Term* t1, Term* t2, int i, int j){
  ...
  getCoeffABC(t1, &a1, &b1, &c1, i, j);
  getCoeffABC(t2, &a2, &b2, &c2, i, j);
  t = new Term((pow(b2, 2) + pow(a2, 2)), 2, j+m, -1);
  Term *last = t;
  last->setNextTerm(new Term((2*b2*c2 - a1*a2*b2 + b1*pow(a2, 2)), 1, j+m, -1));
  last = last->getNextTerm();
     
  last->setNextTerm(new Term((pow(c2, 2) - a1*a2*c2 + c1*pow(a2, 2)), 0, -1, -1));
  if (!solveQuadrEq(t, j))
    return 0;
...
double Prover::computeTerm(Term *t){
  ...
  if (expo == 0){
    ...
    if (expo == 2)
      return (t->getCoeff() * pow(values[ind1], 2));
    else if (ind2 != -1) 
...
int Prover::vanishThesis(){
  ...
  while(th){
	...
	tot = tot + coeff * pow(values[ind1], 2);
...
int main(int argc, char *argv[]){
  ...
  if (th->VerifyByExamples()){
    double prob = 1 - pow(float(2), -numb);
    cout << "\n";
