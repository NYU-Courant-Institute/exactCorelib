/* **************************************
   ...
   Description: 
	This is our implementation of the Plantinga-Vegter algorithm
	for approximation curves represented by a bi-variate polynomial.
	The polynomial is given as a polynomial.

   Usage:

        > ./pv "curveDef" [Xmin] [Ymin] [Xmax] [Ymax] [max-size] [min-size] [offset] [drawbox]

		Note that [...] indicates optional arguments.
		The default values are:
              		The Defualt Curve is [x^2 - x^4 - y^2 +0.01]
	                The Defualt box is   [-1.5,-1.5,1.5,1.5]
					The Defualt max-size is   100000
					The Defualt min-size is   0.0001
	...
	> ./pv "y - x^2" 	        -- plots a parabola 

	> ./pv "y^2 = x^3 - 3 xy" 	-- plots a cubic 
				(note we can use equality sign in the equation)

	> ./pv "x^3y + x(y-2x) = 1" -1 -1 3 3
				-- plots the curve in the box [-1,-1,3,3]

	> ./pv "y - x^2" -10 -10 10 10 100000 0.0001 0.07
   ...
   $Id: PV.cpp,v 1.5 2010/08/05 02:48:12 exact Exp $
 ************************************** */
...
//		ImplicitCurve tmp;
//		value = tmp.value(x, y);
//		Bipolyno.eval(x,y);  // this did not work -- Core2 did not implement "pow" for NT=Expr.
		value = Bipolyno.yExprPolynomial(x).eval(y);  // this is a hack because BiPolyno.eval(x,y) does not work.
		if (value<0)
			return -1;
		if (value>0)
			return 1;
...
// =========================================================
// Interval Class 
// Todo: include the Interval class in Core Lib Instead
// =========================================================
Interval::Interval(){
	a=0;
	b=0;
		...
		for (unsigned int i=0; i<p.size(); i++){
			if (p.at(i).sign()!=p.at((i+1)%4).sign()){
				if (!existV(p.at(i), p.at((i+1)%4))){
					...
					if (v->getX()==-0.025&&v->getY()==0.1){
						cout<<i<<" "<<(i+1)%4<<endl;
						p.at(i).print();
						p.at((i+1)%4).print();
						cout<<p.at(i).sign()<<endl;
						cout<<p.at((i+1)%4).sign()<<endl;
					}
...
void mainFunc::priorityQueue(vector<box *> &vec, box *b){
	if (vec.size() == 0){
		...
		if (b->getSize()<=(*(vec.end()-vector<box *>::iterator::difference_type(1)))->getSize()){
			vec.push_back(b);
		}
	}
...
int mainFunc::run(){
	...
	while (!Q0.empty()){
		...
		else if ((b->c1())&&(b->getSize()<Max_Size)&&(b->getSize()>Min_Size)){
			priorityQueue(Q1,b);
		}
		else{
			vector<box *> s;
...
//	cout<<Q0.size()<<" "<<C0.size()<<" "<<Q1.size()<<" "<<counter<<endl;	
	...
	while (!Q1.empty()){
		...
		for (unsigned int i=0; i<nt.size(); i++){
			if (nt.at(i)->getSize()>2.0*b->getSize()){
				break;
				done=false;
				erase(Q1,nt.at(i));
...
//	cout<<Q1.size()<<" "<<C0.size()<<" "<<Q2.size()<<" "<<counter<<endl;
//	cout<<counter<<endl;
	return 1;
}
...
//			g.drawRect(cord(b.getPoint().get(1).getX()), cord(b.getPoint().get(1).getY()), cordL(b.getX().length()), cordL(b.getY().length()));
	...
	while(!Q2.empty()){
			...
			if (size==2){
				...
				double x2=p.at(1)->getX();
				double y2=p.at(1)->getY();
				Line.push_back(x1);
				Line.push_back(y1);
				Line.push_back(x2);
				Line.push_back(y2);
...
int main(int argc, char* argv[])
{
	...
	if (argc < 2){
	   string CurveDef = "x^2 - x^4 - y^2 + 1";
	   Bipolyno = Bipolyno.getbipoly(CurveDef);
	   Bipolyno.dump("Input Curve is");
	   cout<<"-------------"<<endl;
	   Bipolyno.coeffX[0].setCoeff(0,BigFloat(0.01));
	   Bipolyno.dump("Now Input Curve is");
	   cout<<"-------------"<<endl;
	   box_v1 = -1.5;
	   box_v2 = -1.5;
	   box_v3 = 1.5;
	...
	if (argc>2&&argc<6){
	   cout << "./pv \"curveDef\" [Xmin] [Ymin] [Xmax] [Ymax] [max-size] [min-size] [offset]  "<< endl
		<< "The Defualt Curve is \"x^2 - x^4 - y^2 +0.01\" " << endl
	    << "The Defualt box is \"-1.5,-1.5,1.5,1.5\" " << endl
	...
	glutCreateWindow("PV        (Press ESC to exit)");
	background();
