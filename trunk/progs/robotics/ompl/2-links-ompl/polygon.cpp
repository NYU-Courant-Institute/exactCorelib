...
//}
//#else
//#include "./FIST/fpkernel.h"
//#include "./FIST/martin.h"
//#include "./FIST/defs.h"
//#include "./FIST/api_fist.h"
//#endif
    ...
    bool c_ply::enclosed(const Point2d& p)
    {
		...
		int tsize=triangulation.size();
        for(uint i=0;i<tsize;i++){
            triangle_ & tri=triangulation[i];
    ...
    Point2d c_ply::findEnclosedPt()
    {
        ...
        triangle_ & tri=triangulation[largest_tri];
    ...
    void c_ply::triangulate(vector<triangle_>& tris)
    {
         if(triangulation.empty()){
             ...
             FIST_2D_PolyArray(1, ringVN, (double (*)[2])V, &tN, (int (*)[3])T);
    ...
    void c_polygon::triangulate(vector<triangle_>& tris)
    {
        if(triangulation.empty())
        {
            if(valid()){
                 ...
                 FIST_2D_PolyArray(ringN, ringVN, (double (*)[2])V, &tN, (int (*)[3])T);
                ...
                for(list<c_polygon>::iterator i=subp.begin();i!=subp.end();i++){
                    ...
                    int size=i->triangulation.size();
                    for(uint j=0;j<size;j++){
                        triangle_ tri=i->triangulation[j];
    ...
    ostream& operator<<( ostream& os, c_ply& p)
    {
        ...
        for(uint i=0;i<p.getSize();i++) os<<i+1<<" ";
        os<<"\n";
        return os;
    }
