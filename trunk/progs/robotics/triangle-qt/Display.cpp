...
//            std::cout<<"printf: "<< duration <<'\n';
        ...
        //        0,0,0,true  );
        if (boxA) {
            genRobot(boxA, alpha[0], alpha[1], alpha[2]);
        }
        if (boxB) {
            genRobot(boxB, beta[0], beta[1], beta[2]);
        }
...
/*
 * GENERATE Triangle Robot
 *
 * Generate white lines representing obsticals
 */
void Display::genRobot(Box* b)
{
    double red = 1.0f, green = 0.0f, blue = 0.0f;
...
void Display::genRobot(Box* b, double x, double y, double a)
{
    double red = 1.0f, green = 0.0f, blue = 0.0f;

    // 1st Corner: lower left
    quads.push_back(R0*cos((a) * PI) + x);
    quads.push_back(R0*sin((a) * PI) + y);
    setVertexColor(quads, red, green, blue);

    // 2nd Corner: lower right
    quads.push_back(R0*cos((triRobo[0] + a) * PI) + x);
    quads.push_back(R0*sin((triRobo[0] + a) * PI) + y);
    setVertexColor(quads, red, green, blue);

    // 3rd Corner: upper right
    quads.push_back(R0*cos((triRobo[1] + a) * PI) + x);
    quads.push_back(R0*sin((triRobo[1] + a) * PI) + y);
    setVertexColor(quads, red, green, blue);
