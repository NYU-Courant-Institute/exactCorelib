...
//////////////////////////////////////////////////////////////////////////////////
    double alpha[4] = {250, 350, 0, 0};		// start configuration
    double beta[4] = {20, 50, 0, 0};		// goal configuration
...
int main(int argc, char* argv[]) {
    ...
    // Test if the build directory is triangle. If so,
    ...
    if (indexOfDesiredDir != std::string::npos) {
        ...
        // Set current working directory to triangle
    ...
    // /triangle-qt/ instead of /2-links
    if (!foundFiles &&
    ...
    // will reside in the same directory as /triangle-qt/ or /triangle-qt/
    if (!foundFiles &&
        ...
        if (dir.exists("triangle-qt/triangle-qt.pro")) {                  // Test if /triangle-qt/ exists
            ...
            foundFiles = true;
        } else if (dir.exists("triangle-qt/triangle-qt.pro")) {  // Test if /triangle-qt exists
            workingDir = workingDir.substr(0, indexOfDesiredDir) + "/triangle-qt";
    ...
    // Neither /triangle nor /triangle-qt could be found
    if (!foundFiles) {
        ...
        "If it is not, rename the folder to \"triangle-qt\" before running again.\n";
    }
...
//		mw_out << "interactive : " << interactive << endl;
    ...
    if (QType == 0 || QType == 1) {
        boxA = QT->getBox(alpha[0], alpha[1], alpha[2], 0, ct);
        if (!boxA) {
            noPath = true;
        ...
        boxB = QT->getBox(beta[0], beta[1], beta[2], 0, ct);
        if (!boxB) {
            noPath = true;
