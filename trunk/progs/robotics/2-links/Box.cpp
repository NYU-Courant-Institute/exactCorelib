...
bool Box::splitAngle(double epsilon, vector<Box*>& chldn) {
	...
	for (int i = 0; i < 3; i += 2) {
		children[0]->xi[i] = this->xi[i];
		children[0]->xi[i + 1] = this->xi[i] + (this->xi[i + 1] - this->xi[i]) / 2;
		children[1]->xi[i] = this->xi[i] + (this->xi[i + 1] - this->xi[i]) / 2;
		children[1]->xi[i + 1] = this->xi[i + 1];
	}
