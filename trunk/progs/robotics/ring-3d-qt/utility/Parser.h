...
void parseVertices(vector<Point3d> &pts,
                   ifstream &ifs) {
  int nPt = 0;
  ifs >> nPt;
  //cout << "nPt=" << nPt << endl;
...
void parseCfgFile() {
    ...
    while (fgets(tmp, 200, fptr) != NULL){
        ...
        if (strcmp(sptr, "startZ") == 0) {
            sptr = strtok(NULL, "=: ");
            start.setZ(atof(sptr));
        }
        if (strcmp(sptr, "startRoll") == 0) {
            sptr = strtok(NULL, "=: \t");
            startRot.setX(atof(sptr));
        }
        if (strcmp(sptr, "startPitch") == 0) {
            sptr = strtok(NULL, "=: \t");
            startRot.setY(atof(sptr));
        }
        if (strcmp(sptr, "startYaw") == 0) {
            sptr = strtok(NULL, "=: ");
        ...
        if (strcmp(sptr, "goalZ") == 0) {
            sptr = strtok(NULL, "=: \t");
            goal.setZ(atof(sptr));
        }
        if (strcmp(sptr, "goalRoll") == 0) {
            sptr = strtok(NULL, "=: \t");
            goalRot.setX(atof(sptr));
        }
        if (strcmp(sptr, "goalPitch") == 0) {
            sptr = strtok(NULL, "=: \t");
            goalRot.setY(atof(sptr));
        }
        if (strcmp(sptr, "goalYaw") == 0) {
            sptr = strtok(NULL, "=: \t");
...
void parseConfigFile(ConfBox3d* b) {
  ...
  while (fgets(tmp, 200, fptr) != NULL){
      ...
      if(strcmp(sptr, "Points") == 0){
          while (fgets(tmp, 200, fptr) != NULL){
              ...
              if(sptr[0] > '9' || sptr[0] < '0'){
                  ...
                  sptr = strtok(NULL, "=: \n\t");
                  double z = atof(sptr);
                  Corner* tmpCorner = new Corner(Point3d(x*scale, y*scale, z*scale));
          ...
          while (fgets(tmp, 200, fptr) != NULL){
              ...
              if(strcmp(sptr, "PutPoly") == 0 || strcmp(sptr, "PutPolyhedron") == 0){
                  break;
              }
              if(strcmp(sptr, "End") == 0){
                  break;
      ...
      else if(strcmp(sptr, "PutPoly") == 0 || strcmp(sptr, "PutPolyhedron") == 0){
          sptr = strtok(NULL, "=: \n\t");
          string tmpName = sptr;
          double sign = 1;
          double tx = 0, ty = 0, tz = 0; // translation +, -
      ...
      for (list<Wall*>::iterator iterW = b->Walls.begin(); iterW != b->Walls.end(); ++iterW) {
         ...
         if((*iterE)->same((*iterW)->b, (*iterW)->a) ||
            ...
            (*iterE)->same((*iterW)->a, (*iterW)->c)){
             (*iterE)->oppositeWall = new Wall((*iterW)->a, (*iterW)->b, (*iterW)->c);
         }
      }
  }
    ...
    for (int i = 0; i < numFaces; ++i) {
      ...
      if (objType == 0) {
        ...
        for (int j=0; j< 3; ++j) {
          ...
          ptVec.push_back(new Corner(Point3d(pts[pt].X() * scale,
                                             pts[pt].Y() * scale,
                                             pts[pt].Z() * scale)));
        ...
        ifs >> radius;
        b->addSphere(new Sphere(Point3d(pts[pt].X() * scale,
                                        pts[pt].Y() * scale,
                                        pts[pt].Z() * scale),
                                radius));
      }
