...
GeomObj* Segment3d::intersection( const Segment3d& s ) const {
  ...
  if( toLine().isCoincident( s.toLine() ) ) {
    ...
    if( p!=NULL && contains(*p) && s.contains(*p) )    // check, will be inaccurate
      return p;
    else 
      return NULL;
  }
...
//  Vector p1p0_(p1_-p0_); // p1'-p0' vector

//  double t = Vector(cir.P()-p0_)*p1p0_/(p1p0_*p1p0_);
//  if(t < 0 || t > 1)
//    return ;

//  Point3d p = p0+t*direction();
//  seg_p = new Point3d(p);
//  seg_p->separation_circle(cir, cir_p);
}
...
//  Point3d p = p0+t*direction();
//  return p.separation_circle(cir);
}
...
//  Point3d p = p0+t*direction();
//  return p.separation_circleL(cir);
}
