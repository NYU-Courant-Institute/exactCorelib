...
string robotDir("robots");
string robotName("L.rob");
double robotMinMaxRadiusRatio;

double alpha[3] = { 400, 320, 0 };		// start configuration
double beta[3] = { 100, 70, 0 };         // goal configuration
...
bool loadComplexRobot(string& robot_file, vector<Triangle>& compRoboTri,
  ...
  if (infile >> nt) { // if manually input triangles
    for (int i = 0; i < nt; ++i)  {
      ...
      // decompose into nice triangles
      vector<Triangle> nice_triangles = compRoboTri.back().niceDecompose();
      if (nice_triangles.size() > 1) {
          compRoboTri.pop_back();
          compRoboTri.insert(compRoboTri.end(), nice_triangles.begin(), nice_triangles.end());
      }
    }
  }
  ...
  for (Triangle& tri: compRoboTri) {
      mindist = min(mindist, tri.a.x*tri.a.x+tri.a.y*tri.a.y);
      mindist = min(mindist, tri.a.x*tri.a.x+tri.a.y*tri.a.y);
      mindist = min(mindist, tri.a.x*tri.a.x+tri.a.y*tri.a.y);
  }
  robotMinMaxRadiusRatio = mindist = sqrt(mindist);
...
void run() {
    ...
    if (!loadComplexRobot(robot_file, compRoboTriangles, robotMinMaxRadiusRatio)){
        return;
    }
