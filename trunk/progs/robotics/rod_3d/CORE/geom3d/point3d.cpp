...
double signed_volume(const Point3d& a, const Point3d& b, const Point3d& c,
                     const Point3d& d) {
  double a11 = a.x - d.x;
  double a12 = a.y - d.y;
  double a13 = a.z - d.z;
  ...
  double s = a11 * (a22 * a33 - a23 * a32) - a12 * (a21 * a33 - a23 * a31) +
             a13 * (a21 * a32 - a22 * a31);
  return s;
}
...
double volume(const Point3d& a, const Point3d& b, const Point3d& c,
              const Point3d& d) {
...
bool Point3d::intersectHalfspaces(const std::vector<Plane3d>& half_spaces,
  ...
  for (const auto& half_space : half_spaces) {
    if (verbose)
      fprintf(debug_ptr, "apply %f\n", half_space.apply(Point3d(x, y, z)));
    // apply the point to the plane equation
    if (half_space.apply(Point3d(x, y, z)) > 0) {
      return false;
    }
  }
  return true;
...
double Point3d::separation(const Segment3d& s) const {
  Point3d p(x, y, z);
  Vector v(s.stopPt() - s.startPt());
  Vector w(p - s.startPt());

  double c1 = w * v;
  if (c1 <= 0) return p.distance(s.startPt());

  double c2 = v * v;
  if (c2 <= c1) return p.distance(s.stopPt());

  double b = c1 / c2;
  Point3d p_ = s.startPt() + b * v;
