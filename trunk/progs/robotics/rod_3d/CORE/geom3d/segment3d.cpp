...
Segment3d::Segment3d(const Point3d& _p0, const Point3d& _p1)
    : p0(_p0), p1(_p1) {
  directed = false;
  open = false;
}
...
bool Segment3d::isCoincident(const Segment3d& s) const {
  ...
  else if ((p0 == s.startPt() && p1 == s.stopPt()) ||
           (p1 == s.startPt() && p0 == s.stopPt()))
    return true;
  else
    return false;
}
 ...
 */
Point3d Segment3d::nearPoint(const Point3d& p) const {
...
double Segment3d::separation(const Segment3d& s) const {
...
int Segment3d::intersects(const Segment3d& s) const {
  ...
  if (res == -1)
    ...
    double s4 =
        dotProduct(p1 - s.startPt(), p1 - s.stopPt());  // *<---------*<--o
...
GeomObj* Segment3d::intersection(const Line3d& l) const {
  if (!isCoplanar(l)) return NULL;

  if (l.isCoincident(toLine())) return new Segment3d(*this);
...
// 	of the cone) is simpler: it is just the ray from
...
bool Segment3d::intersectHalfspaces(const std::vector<Plane3d>& half_spaces,
  ...
  if (verbose)
    fprintf(
        debug_ptr,
        "current segment %f %f %f  %f %f %f  closest point %f %f %f  %d %d "
        "%d\n",
...
std::ostream& operator<<(std::ostream& out, const Segment3d& s) {
  return out << "Segment3d[" << s.p0 << "," << s.p1
             << "; directed=" << s.directed << "; open=" << s.open << "]";
}
