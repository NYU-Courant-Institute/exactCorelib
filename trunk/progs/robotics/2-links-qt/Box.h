    ...
    void distribute_features2box(Box * child) {
        ...
        for (WIT iterW = vorWalls.begin(); iterW != vorWalls.end(); ++iterW) {
            ...
            if (dist < cl2r) {//within the distance range
                ...
                if (zone) child->vorWalls.push_back(w);
            } //end if
        } //end for
        ...
        for (CIT iterC = vorCorners.begin(); iterC != vorCorners.end(); ++iterC) {
            ...
            if (dist < cl2r) {
                ...
                if (zone) child->vorCorners.push_back(c);
            } //end if
        } //end for
...
//
        ...
        //
        // compute a closest corner that is closer than mindistW
        //
        ...
        if (fabs(mindistW - mindistC) < 1e-10) {
            if (in_zone_w) {
