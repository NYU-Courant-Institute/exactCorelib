...
int main(int argc, char* argv[]) {
    ...
    // include /2-links
    int indexOfDesiredDir = workingDir.rfind("/2-links/");
    ...
    if (!foundFiles &&
        (indexOfDesiredDir = workingDir.rfind("/2-links-master/")) != std::string::npos) {
    ...
    // will reside in the same directory as /2-links or /2-links-master
    if (!foundFiles &&
        (indexOfDesiredDir = workingDir.rfind("/build-2-links-")) != std::string::npos) {
        QDir dir(workingDir.substr(0, indexOfDesiredDir).c_str());

        if (dir.exists("2-links/2-links.pro")) {                  // Test if /2-links exists
            workingDir = workingDir.substr(0, indexOfDesiredDir) + "/2-links";

            // Set current working directory to 2-links
            QDir::setCurrent(workingDir.c_str());

            foundFiles = true;
        } else if (dir.exists("2-links-master/2-links.pro")) {  // Test if /2-links-master exists
            workingDir = workingDir.substr(0, indexOfDesiredDir) + "/2-links-master";
    ...
    // Neither /2-links nor /2-links-master could be found
    if (!foundFiles) {
        ...
        "Make sure that the program is inside of a folder named \"2-links\".\n"
        "If it is not, rename the folder to \"2-links\" before running again.\n";
    }
...
////		mw_out<<"before dijkstraShortestPath"<<endl;
      PATH = graph.dijkstraShortestPath(boxA, boxB);
////		mw_out<<"after dijkstraShortestPath"<<endl;
    }
    if (verboseOption)
        mw_out << ">>>>>>>>>>>>>>> > > > > > > >>>>>>>>>>>>>>>>>>\n";
    mw_out << ">>\n";
