...
Status ConfBox3dPredicate::checkChildStatus(ConfBox3d* b, double x, double y, double z, int boxId) {
  Wall* nearestWall;
...
void ConfBox3dPredicate::classify(ConfBox3d* b) {
  ...
  if (b->corners.empty() && b->Edges.empty() && b->Walls.empty() && b->spheres.empty()) {
    if (!b->parent) {
      ...
      b->status = checkChildStatus(b->parent, b->x, b->y, b->z, b->boxId);
    }
  }
