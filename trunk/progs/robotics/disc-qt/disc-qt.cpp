...
//////////////////////////////////////////////////////////////////////////////////
double alpha[4] = { 10, 360, 0, 0 };		// start configuration
double beta[4] = { 500, 20, 0, 0 };		// goal configuration
...
int main(int argc, char* argv[]) {
    ...
    // Test if the build directory is disc-qt. If so,
    ...
    // /2-links-master instead of /disc-qt
    if (!foundFiles &&
    ...
    // Test if a build directory (/build-disc-qt-...) was created. This directory
    // will reside in the same directory as /disc-qt or /disc-qt
    if (!foundFiles &&
        (indexOfDesiredDir = workingDir.rfind("/build-disc-qt")) != std::string::npos) {
        QDir dir(workingDir.substr(0, indexOfDesiredDir).c_str());

        if (dir.exists("disc-qt/disc-qt.pro")) {                  // Test if /disc-qt exists
            ...
            foundFiles = true;
        } else if (dir.exists("disc-qt-master/disc-qt.pro")) {  // Test if /disc-qt-master exists
            workingDir = workingDir.substr(0, indexOfDesiredDir) + "/disc-qt-master";
    ...
    // Neither /disc-qt nor /disc-qt could be found
    if (!foundFiles) {
        ...
        "Make sure that the program is inside of a folder named \"disc-qt-master\".\n"
        "If it is not, rename the folder to \"disc-qt-master\" before running again.\n";
    }
...
void run() {
////	mw_out << "1111111111111111111111111111111111111111111111111" << endl;
