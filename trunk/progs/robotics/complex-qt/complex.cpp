/* **************************************
 File: complex.cpp	-- non-interactive version of complex-qt.cpp
...
// a 2d complex robot is defined by a list of vertices
...
int main(int argc, char* argv[]) {
    ...
    if (interactive > 0) {	// non-interactive
        // do something...
        cout << "Non Interactive Run of Complex Robot" ;
    }
    ...
    if (interactive > 0) {	// non-interactive
        if (noPath) std::cout << "No Path Found!\n";
        else        std::cout << "Path was Found!\n" ;
        return 0;
    }
...
void genEmptyTree() {
    ...
    if (verboseOption)  std::cout << "done genEmptyTree \n";
}


void run() {
    std::cout << "Run No. " << runCount++ << "\n";
    ...
    // bo debug
    string robot_file = workingDir + "/" + robotDir + "/" + robotName;
    ...
    if (verboseOption) {
        std::cout << "   radius = " << R0 << ", eps = " << epsilon<<"\n" ;
    ...
    if (QType == 0 || QType == 1) {
        ...
        if (!boxA) {
            noPath = true;
            std::cout << "Start Configuration is not free\n";
        }

        boxB = QT->getBox(beta[0], beta[1], beta[2], ct);
        if (!boxB) {
            noPath = true;
            std::cout << "Goal Configuration is not free\n";
        }
    ...
    std::cout << ">>\n";
    std::cout << ">>      ----->>  Time used: " << t.getElapsedTimeInMilliSec()
            << " ms\n" ;
    //std::cout << ">>      ----->>  Expansion steps: ";
    //std::cout << (int) expansions.size()-1;
    //std::cout << "\n";
    std::cout << ">>\n";
    ...
    case 0:
        ...
        break;
    case 1:
        std::cout << "BFS Strategy\n";
        break;
    case 2:
        std::cout << "Greedy Strategy\n";
        break;
    case 3:
        std::cout << "Dist+Size Strategy\n";
        break;
    case 4:
        std::cout << "Voronoi Strategy\n";
        break;
    }
    std::cout << ">>\n";
    if (verboseOption)
        std::cout << ">>>>>>>>>>>>>>> > > > > > > >>>>>>>>>>>>>>>>>>\n";
    if (verboseOption) {
        std::cout << "    Expanded " << ct << " times\n" ;
        std::cout << "    total Free boxes: " << freeCount<<"\n" ;
        std::cout << "    total Stuck boxes: " << stuckCount <<"\n";
        std::cout << "    total Mixed boxes smaller than epsilon: " << mixSmallCount<<"\n";

        std::cout << "    total Mixed boxes bigger than epsilon: "
...
void parseConfigFile(Box* b) {
    ...
    ifs.close();
    if (true) {
        std::cout << "input file name = " << s <<"\n";
        std::cout << "nPt=" << nPt<<"\n";
        std::cout << "nPolygons=" << nPolygons <<"\n";
    }
}		//parseConfigFile
