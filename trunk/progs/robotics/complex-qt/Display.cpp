...
void Display::drawObstacles() {
    ...
    for (unsigned int k = 0; k < b->walls.size(); k++) {
        for (list<Wall*>::iterator iter = b->walls[k].begin(); iter != b->walls[k].end(); ++iter) {
            Wall* w = *iter;
...
void Display::drawRobot(Pose p, Color c){
    ...
    for (Triangle & tri:compRoboTriangles) {
        ...
        Corner A = Corner(tri.a.x* R0, tri.a.y* R0);
        A.rotate(p.theta * PI,0,0);
        A = A + boxcenter;
        Corner B = Corner(tri.b.x* R0, tri.b.y* R0);
        B.rotate(p.theta * PI,0,0);
        B = B + boxcenter;
        Corner C = Corner(tri.c.x* R0, tri.c.y* R0);
