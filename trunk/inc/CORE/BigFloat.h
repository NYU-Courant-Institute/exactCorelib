...
public:
  ...
  //@{
  /// default constructor
  BigFloat() : m_l(0), m_exact(true)
  {}
  /// copy constructor
  BigFloat(const BigFloat& r) : m_l(r.m_l), m_r(r.m_r), m_exact(r.m_exact)
  {}
  /// generic constructor for <tt>T</TT>
  template <typename T> BigFloat(const T& v) : m_l(v), m_exact(true)
  {}
  ...
  explicit BigFloat(const std::string& str, int base = 10) 
  { set(str, base); }
  //@}
...
public: 
  ...
  //@{
  /// assignment function for <tt>BigFloat</tt>
  bool r_set(const BigFloat& x)
  ...
  //@{
  /// assignment function for <tt>BigFloat</tt>
  bool set(const BigFloat& x, prec_t prec)
  ...
  //@{
  /// negation for <tt>BigFloat</tt>
  bool r_neg(const BigFloat& x)
  ...
  /// square root for <tt>BigFloat</tt>
  bool r_sqrt(const BigFloat& x)
  ...
  /// cubic root for <tt>BigFloat</tt>
  bool r_cbrt(const BigFloat& x)
  ...
  /// k-th root for <tt>BigFloat</tt>
  bool r_root(const BigFloat& x, unsigned long k)
  ...
  bool r_addsub(const BigFloat& x, const BigFloat& y, bool is_add)
  { return is_add ? r_add(x, y) : r_sub(x, y); }
  /// addition/subtraction for <tt>BigFloat, T</tt>
  template <typename T>
  bool r_addsub(const BigFloat& x, const T& y, bool is_add)
  { return is_add ? r_add(x, y) : r_sub(x, y); }
  /// addition/subtraction for <tt>T, BigFloat</tt>
  template <typename T>
  bool r_addsub(const T& x, const BigFloat& y, bool is_add)
  { return is_add ? r_add(x, y) : r_sub(x, y); }

  /// addition for <tt>BigFloat+BigFloat</tt>
  bool r_add(const BigFloat& x, const BigFloat& y)
  { return _add<RawArithmeticPolicy>(x, y); }
  /// addition for <tt>BigFloat+T</tt>
  template <typename T> bool r_add(const BigFloat& x, const T& y)
  { return _add<RawArithmeticPolicy, T>(x, y); }
  /// addition for <tt>T+BigFloat</tt>
  template <typename T> bool r_add(const T& x, const BigFloat& y)
  { return _add<RawArithmeticPolicy, T>(x, y); }

  /// subtraction for <tt>BigFloat-BigFloat</tt>
  bool r_sub(const BigFloat& x, const BigFloat& y)
  { return _sub<RawArithmeticPolicy>(x, y); }
  /// subtraction for <tt>BigFloat-T</tt>
  template <typename T> bool r_sub(const BigFloat& x, const T& y)
  { return _sub<RawArithmeticPolicy, T>(x, y); }
  /// subtraction for <tt>T-BigFloat</tt>
  template <typename T> bool r_sub(const T& x, const BigFloat& y)
  { return _sub<RawArithmeticPolicy, T>(x, y); }

  /// multiplication for <tt>BigFloat*BigFloat</tt>
  bool r_mul(const BigFloat& x, const BigFloat& y)
  { return _mul<RawArithmeticPolicy>(x, y); }
  /// multiplication for <tt>BigFloat*T</tt>
  template <typename T> bool r_mul(const BigFloat& x, const T& y)
  { return _mul<RawArithmeticPolicy, T>(x, y); }
  /// multiplication for <tt>T*BigFloat</tt>
  template <typename T> bool r_mul(const T& x, const BigFloat& y)
  { return _mul<RawArithmeticPolicy, T>(x, y); }

  /// division for <tt>BigFloat/BigFloat</tt>
  bool r_div(const BigFloat& x, const BigFloat& y)
  { return _div<RawArithmeticPolicy>(x, y); }
  /// division for <tt>BigFloat/T</tt>
  template <typename T> bool r_div(const BigFloat& x, const T& y)
  { return _div<RawArithmeticPolicy, T>(x, y); }
  /// division for <tt>T/BigFloat</tt>
  template <typename T> bool r_div(const T& x, const BigFloat& y)
  { return _div<RawArithmeticPolicy, T>(x, y); }
  //@}
  ...
  //@{
  /// negation for <tt>BigFloat</tt>
  bool neg(const BigFloat& x, prec_t prec)
  ...
  /// square root for <tt>BigFloat</tt>
  bool sqrt(const BigFloat& x, prec_t prec = BF_DEF_SQRT_PREC)
  ...
  /// cubic root for <tt>BigFloat</tt>
  bool cbrt(const BigFloat& x, prec_t prec = BF_DEF_CBRT_PREC)
  ...
  /// k-th root for <tt>BigFloat</tt>
  bool root(const BigFloat& x, unsigned long k, prec_t prec = BF_DEF_ROOT_PREC)
  ...
  /// addition/subtraction for <tt>BigFloat</tt>
  bool addsub(const BigFloat& x, const BigFloat& y, prec_t prec, bool is_add)
  { return is_add ? add(x, y, prec) : sub(x, y, prec); }
  /// addition/subtraction for <tt>BigFloat, T</tt>
  template <typename T>
  bool addsub(const BigFloat& x, const T& y, prec_t prec, bool is_add)
  { return is_add ? add(x, y, prec) : sub(x, y, prec); }
  /// addition/subtraction for <tt>T, BigFloat</tt>
  template <typename T>
  bool addsub(const T& x, const BigFloat& y, prec_t prec, bool is_add)
  { return is_add ? add(x, y, prec) : sub(x, y, prec); }

  /// addition for <tt>BigFloat+BigFloat</tt>
  bool add(const BigFloat& x, const BigFloat& y, prec_t prec)
  { return _add<FixedArithmeticPolicy>(x, y, prec); }
  /// addition for <tt>BigFloat+T</tt>
  template <typename T> bool add(const BigFloat& x, const T& y, prec_t prec)
  { return _add<FixedArithmeticPolicy, T>(x, y, prec); }
  /// addition for <tt>T+BigFloat</tt>
  template <typename T> bool add(const T& x, const BigFloat& y, prec_t prec)
  { return _add<FixedArithmeticPolicy, T>(x, y, prec); }

  /// subtraction for <tt>BigFloat-BigFloat</tt>
  bool sub(const BigFloat& x, const BigFloat& y, prec_t prec)
  { return _sub<FixedArithmeticPolicy>(x, y, prec); }
  /// subtraction for <tt>BigFloat-T</tt>
  template <typename T> bool sub(const BigFloat& x, const T& y, prec_t prec)
  { return _sub<FixedArithmeticPolicy, T>(x, y, prec); }
  /// subtraction for <tt>T-BigFloat</tt>
  template <typename T> bool sub(const T& x, const BigFloat& y, prec_t prec)
  { return _sub<FixedArithmeticPolicy, T>(x, y, prec); }

  /// multiplication for <tt>BigFloat*BigFloat</tt>
  bool mul(const BigFloat& x, const BigFloat& y, prec_t prec)
  { return _mul<FixedArithmeticPolicy>(x, y, prec); }
  /// multiplication for <tt>BigFloat*T</tt>
  template <typename T> bool mul(const BigFloat& x, const T& y, prec_t prec)
  { return _mul<FixedArithmeticPolicy, T>(x, y, prec); }
  /// multiplication for <tt>T*BigFloat</tt>
  template <typename T> bool mul(const T& x, const BigFloat& y, prec_t prec)
  { return _mul<FixedArithmeticPolicy, T>(x, y, prec); }

  /// division for <tt>BigFloat/BigFloat</tt>
  bool div(const BigFloat& x, const BigFloat& y, prec_t prec = BF_DEF_DIV_PREC)
  { return _div<FixedArithmeticPolicy>(x, y, prec); }
  /// division for <tt>BigFloat/T</tt>
  template <typename T> 
  bool div(const BigFloat& x, const T& y, prec_t prec = BF_DEF_DIV_PREC)
  { return _div<FixedArithmeticPolicy, T>(x, y, prec); }
  /// division for <tt>T/BigFloat</tt>
  template <typename T> 
  bool div(const T& x, const BigFloat& y, prec_t prec = BF_DEF_DIV_PREC)
  { return _div<FixedArithmeticPolicy, T>(x, y, prec); }
  //@}
  ...
  /// addition/subtraction for <tt>BigFloat</tt>
  bool addsub(const BigFloat& x, const BigFloat& y, bool is_add)
  { return is_add ? add(x, y) : sub(x, y); }
  /// addition/subtraction for <tt>BigFloat, T</tt>
  template <typename T>
  bool addsub(const BigFloat& x, const T& y, bool is_add)
  { return is_add ? add(x, y) : sub(x, y); }
  /// addition/subtraction for <tt>T, BigFloat</tt>
  template <typename T>
  bool addsub(const T& x, const BigFloat& y, bool is_add)
  { return is_add ? add(x, y) : sub(x, y); }

  /// addition for <tt>BigFloat+BigFloat</tt>
  bool add(const BigFloat& x, const BigFloat& y)
  { return _add<AutoArithmeticPolicy>(x, y); }
  /// addition for <tt>BigFloat+T</tt>
  template <typename T> bool add(const BigFloat& x, const T& y)
  { return _add<AutoArithmeticPolicy, T>(x, y); }
  /// addition for <tt>T+BigFloat</tt>
  template <typename T> bool add(const T& x, const BigFloat& y)
  { return _add<AutoArithmeticPolicy, T>(x, y); }

  /// subtraction for <tt>BigFloat-BigFloat</tt>
  bool sub(const BigFloat& x, const BigFloat& y)
  { return _sub<AutoArithmeticPolicy>(x, y); }
  /// subtraction for <tt>BigFloat-T</tt>
  template <typename T> bool sub(const BigFloat& x, const T& y)
  { return _sub<AutoArithmeticPolicy, T>(x, y); }
  /// subtraction for <tt>T-BigFloat</tt>
  template <typename T> bool sub(const T& x, const BigFloat& y)
  { return _sub<AutoArithmeticPolicy, T>(x, y); }

  /// multiplication for <tt>BigFloat*BigFloat</tt>
  bool mul(const BigFloat& x, const BigFloat& y)
  { return _mul<AutoArithmeticPolicy>(x, y); }
  /// multiplication for <tt>BigFloat*T</tt>
  template <typename T> bool mul(const BigFloat& x, const T& y)
  { return _mul<AutoArithmeticPolicy, T>(x, y); }
  /// multiplication for <tt>T*BigFloat</tt>
  template <typename T> bool mul(const T& x, const BigFloat& y)
  { return _mul<AutoArithmeticPolicy, T>(x, y); }
  //@}
...
#ifndef NDEBUG
      std::cerr << "BigFloat Warning: cannot get correct sign!" << std::endl;
#endif
      return 0;
    }
  }
...
public: // C++ style operators
  ...
  template <typename T> BigFloat& operator=(const T& rhs)
  { set(rhs); return *this; }
  /// generic compound assignment operator <tt>+=</tt>
  template <typename T> BigFloat& operator+=(const T& rhs) 
  { BigFloat t; t.add(*this, rhs); t.swap(*this); return *this; }
  /// generic compound assignment operator <tt>-=</tt>
  template <typename T> BigFloat& operator-=(const T& rhs) 
  { BigFloat t; t.sub(*this, rhs); t.swap(*this); return *this; }
  /// generic compound assignment operator <tt>*=</tt>
  template <typename T> BigFloat& operator*=(const T& rhs) 
  { BigFloat t; t.mul(*this, rhs); t.swap(*this); return *this; }
  /// generic compound assignment operator <tt>/=</tt>
  template <typename T> BigFloat& operator/=(const T& rhs) 
  { BigFloat t; t.div(*this, rhs); t.swap(*this); return *this; }
  //@}
...
private:
  ...
  bool _set(const BigFloat& x, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy>
  ...
  // negation
  template <template <typename, typename, typename> class Policy>
  bool _neg(const BigFloat& x, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _neg(const T& x, prec_t prec = 0);
  
  // square root
  template <template <typename, typename, typename> class Policy>
  bool _sqrt(const BigFloat& x, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _sqrt(const T& x, prec_t prec = 0);
  
  // cubic root
  template <template <typename, typename, typename> class Policy>
  bool _cbrt(const BigFloat& x, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _cbrt(const T& x, prec_t prec = 0);
  
  // k-th root
  template <template <typename, typename, typename> class Policy>
  bool _root(const BigFloat& x, unsigned long k, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _root(const T& x, unsigned long k, prec_t prec = 0);
  
  // addition
  template <template <typename, typename, typename> class Policy>
  bool _add(const BigFloat& x, const BigFloat& y, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _add(const BigFloat& x, const T& y, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _add(const T& x, const BigFloat& y, prec_t prec = 0);

  // subtraction 
  template <template <typename, typename, typename> class Policy>
  bool _sub(const BigFloat& x, const BigFloat& y, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _sub(const BigFloat& x, const T& y, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _sub(const T& x, const BigFloat& y, prec_t prec = 0);

  // multiplication
  template <template <typename, typename, typename> class Policy>
  bool _mul(const BigFloat& x, const BigFloat& y, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _mul(const BigFloat& x, const T& y, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _mul(const T& x, const BigFloat& y, prec_t prec = 0);

  // division
  template <template <typename, typename, typename> class Policy>
  bool _div(const BigFloat& x, const BigFloat& y, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _div(const BigFloat& x, const T& y, prec_t prec = 0);
  template <template <typename, typename, typename> class Policy, typename T>
  bool _div(const T& x, const BigFloat& y, prec_t prec = 0);
};
...
/// BigFloat / BigFloat (w/ default precision BF_DEF_DIV_PREC)
inline BigFloat operator/(const BigFloat& x, const BigFloat& y)
{ BigFloat r; r.div(x, y); return r; }
/// BigFloat / T (w/ default precision BF_DEF_DIV_PREC)
template <typename T> inline BigFloat operator/(const BigFloat& x, const T& y)
{ BigFloat r; r.div(x, y); return r; }
/// T / BigFloat (w/ default precision BF_DEF_DIV_PREC)
template <typename T> inline BigFloat operator/(const T& x, const BigFloat& y)
{ BigFloat r; r.div(x, y); return r; }
//@}
...
{ BigFloat::FT tmp; is >> tmp; x.set(tmp); return is; }
...
//@{
/// square root
inline BigFloat sqrt(const BigFloat& x, prec_t prec = BF_DEF_SQRT_PREC)
{ BigFloat r; r.sqrt(x, prec); return r; }
/// cubic root
inline BigFloat cbrt(const BigFloat& x, prec_t prec = BF_DEF_CBRT_PREC)
{ BigFloat r; r.cbrt(x, prec); return r; }
/// k-th root
inline BigFloat root(const BigFloat& x, unsigned long k, prec_t prec = BF_DEF_ROOT_PREC)
{ BigFloat r; r.root(x, k, prec); return r; }
//@}
