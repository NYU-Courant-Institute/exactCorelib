 ...
 *
 * $Source: /home/exact/cvsroot/exact/corelib2/inc/CORE/poly/Poly.tcc,v $
...
template <class NT>
Polynomial<NT>::Polynomial(const Polynomial<NT> & p) : coeff(NULL) {
...
//
template <class NT>
BigFloat2 Polynomial<NT>::evalApprox(const BigFloat& f, long r) const { 
  if (degree == -1)
    return BigFloat(0);
  if (degree == 0)
    return BigFloat(coeff[0], r);

  BigFloat2 val(0), c;
  for (int i=degree; i>=0; i--) {
    c = BigFloat(coeff[i], r);	
    val *= f; 
    val += c;
  }
  return val;
...
template <>
inline
BigFloat2 Polynomial<BigInt>::evalApprox(const BigFloat& f, long r) const {
  assert(0);
  return BigFloat(0);
}
 ...
 ***************************************************/
template <class NT>
BigFloat Polynomial<NT>::evalExactSign(const BigFloat& val, long oldMSB) const {
    assert(val.isExact());
    if (getTrueDegree() == -1)
      return BigFloat(0);
  
    long r;
    r = 1 + BigFloat(height()).uMSB() + ceillg(long(getTrueDegree()+1));
    if (val > 1)
      r += getTrueDegree() * val.uMSB();
    r += std::max(0L, -oldMSB);
  
    if (hasExactDivision<NT>::check()) { // currently, only to detect NT=Expr and NT=BigRat
        BigFloat2 rVal = evalApprox(val, r);
        if (rVal.has_zero()) {
	  Expr eVal = eval(Expr(val));	// eval gives exact value
	  eVal.r_approx(54);  // if value is 0, we get exact 0
...
template < class NT >
BigFloat Polynomial<NT>::CauchyBound() const {
  int deg = getTrueDegree();
  ...
  while (true) {
    ...
    for (int i=deg-1; i>=0; i--) {
      lhs = lhs * B + abs(coeff[i]);
      //std::cout << "coeff=" << abs(coeff[i]) << std::endl;
...
BigFloat Polynomial<NT>::sepBound() const {
  ...
  return (BigFloat(1)/(e*2*d)); //.makeFloorExact();
